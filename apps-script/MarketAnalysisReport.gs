var MR_GH = {
  ...
  accessProp: ['GITHUB','MARKET','ACCESS'].join('_'),
...
function buildMarketAnalysisReport() { return startMarketAnalysisReport(); }
...
function fetchMarketAnalysisReport() {
  ...
  var report = JSON.parse(mrReadGitHubFile_(outputPath));
...
function fetchLatestMarketAnalysisReport() {
  var report = JSON.parse(mrReadGitHubFile_('runtime/market-outputs/latest-market-report.json'));
  mrWriteBackendReport_(report);
...
function checkMarketGitHubConfig() {
  ...
  if (!hasAccess) return 'Missing Script Property: ' + MR_GH.accessProp;
...
function setMarketGitHubConfig(accessValue, repoFullName, branch, workflowFile) {
  if (!accessValue) return checkMarketGitHubConfig() + '\nNote: you ran setMarketGitHubConfig without arguments. If Script Properties are already pasted manually, run testMarketGitHubConnection next.';
  var props = PropertiesService.getScriptProperties();
...
function mrBuildGitHubRequestPayload_() {
  ...
  var rows = values.slice(1).filter(function(row) { return row.join('').trim() !== ''; });
  if (!rows.length) throw new Error('No Holdings rows found under the header. Pull holdings first.');
  return { request_id: mrRequestId_(), headers: headers, rows: rows, source: 'Google Sheets Holdings tab', spreadsheet_id: ss.getId(), created_at: mrNow_() };
}
...
function mrTriggerMarketWorkflow_(requestPath) {
  ...
  mrGitHubApi_(endpoint, 'post', { ref: cfg.branch, inputs: { request_path: requestPath } }, true);
}

function mrCreateGitHubFile_(path, content, message) {
  var lastErr = '';
  for (var attempt = 1; attempt <= 6; attempt++) {
    try { return mrCreateGitHubFileOnce_(path, content, message); }
    catch (err) {
      lastErr = err && err.message ? err.message : String(err);
      if (lastErr.indexOf('409') === -1) throw err;
      Utilities.sleep(1200 * attempt);
    }
  }
  throw new Error('GitHub write conflict after retries. Try Start Market Analysis again in 30 seconds. Last error: ' + lastErr);
}

function mrCreateGitHubFileOnce_(path, content, message) {
  var cfg = mrGetGitHubCfg_();
  ...
  return mrGitHubApi_(endpoint, 'put', { message: message, content: Utilities.base64Encode(content), branch: cfg.branch });
}
...
function mrGitHubApi_(endpoint, method, body, allowEmpty) {
  ...
  headers['Auth' + 'orization'] = ['Be','arer '].join('') + cfg.access;
  var options = { method: method || 'get', muteHttpExceptions: true, headers: headers };
  if (body !== undefined && body !== null) { options.contentType = 'application/json'; options.payload = JSON.stringify(body); }
  var res = UrlFetchApp.fetch('https://api.github.com' + endpoint, options);
  var code = res.getResponseCode();
  var text = res.getContentText();
  if (code >= 200 && code < 300) { if (allowEmpty && !text) return {}; if (!text) return {}; try { return JSON.parse(text); } catch (err) { return { raw: text }; } }
...
function mrGetGitHubCfg_() {
  ...
  if (!access) throw new Error('Missing Script Property: ' + MR_GH.accessProp);
  ...
  return { access: access, owner: parts[0], repo: parts[1], full: full, branch: props.getProperty(MR_GH.branchProp) || MR_GH.defaultBranch, workflow: props.getProperty(MR_GH.workflowProp) || MR_GH.defaultWorkflow };
}
...
function mrWriteBackendReport_(report) {
  ...
  sh.getRange(1, 1, sh.getMaxRows(), Math.min(13, sh.getMaxColumns())).setFontFamily(MR_GH.font).setFontSize(9).setWrap(true).setVerticalAlignment('top');
  var row = 1;
  ...
  report.sections.forEach(function(section) { if (!section) return; if (section.type === 'paragraph') { if (section.title) row = mrSection_(sh, row, section.title); row = mrParagraph_(sh, row, section.text || ''); } else if (section.type === 'table') { row = mrSection_(sh, row, section.title || 'Table'); row = mrTable_(sh, row, section.headers || [], section.rows || []); } });
  mrFinalize_(sh, row);
}
...
function mrTable_(sh, row, headers, rows) { headers = headers || []; rows = rows || []; var width = Math.max(headers.length, 1); var cleanRows = rows.map(function(r) { r = r || []; var out = []; for (var i = 0; i < width; i++) out.push(r[i] === undefined || r[i] === null ? '' : r[i]); return out; }); var cleanHeaders = []; for (var h = 0; h < width; h++) cleanHeaders.push(headers[h] || ''); var data = [cleanHeaders].concat(cleanRows); var range = sh.getRange(row, 1, data.length, width); range.setValues(data).setWrap(true).setVerticalAlignment('top').setBorder(true, true, true, true, true, true, '#cbd5e1', SpreadsheetApp.BorderStyle.SOLID); sh.getRange(row, 1, 1, width).setFontWeight('bold').setBackground('#1f2937').setFontColor('#ffffff'); if (data.length > 1) sh.getRange(row + 1, 1, data.length - 1, width).setBackground('#ffffff').setFontColor('#111827'); return row + data.length + 2; }
function mrFinalize_(sh, lastRow) { for (var c = 1; c <= 13; c++) { var w = 110; if (c === 8 || c === 12) w = 220; if (c === 13) w = 135; sh.setColumnWidth(c, w); } sh.getRange(1, 1, Math.max(1, lastRow), Math.min(13, sh.getMaxColumns())).setFontFamily(MR_GH.font); sh.autoResizeRows(1, Math.max(1, lastRow)); }
function mrRequestId_() { return Utilities.formatDate(new Date(), Session.getScriptTimeZone(), 'yyyyMMdd_HHmmss') + '_' + Utilities.getUuid().slice(0, 8); }
