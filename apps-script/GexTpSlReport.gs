...
function buildGexTakeProfitStopLimit(){
  ...
  return 'GEX TP & SL complete. Main rows shown: '+rows.length+'. Phone-Gmail tab updated.'+(errors.length?' Hidden/issues: '+errors.length+'.':'')+' TP % and SL % are based on cost per share, not spot.';
}
...
function gexBuildPhoneGmailTab_(ss,rows){var sh=ss.getSheetByName(GEX_TPSL.phoneSheet)||ss.insertSheet(GEX_TPSL.phoneSheet);gexPhonePrepare_(sh);var r=1;r=gexPhoneTitle_(sh,r,'GEX TP / SL','Phone-Gmail view • '+gexNow_());r=gexPhoneSummary_(sh,r,rows.length);rows.forEach(function(x,i){r=gexPhoneCard_(sh,r,gexPhoneObj_(x),i+1);});gexPhoneFinalize_(sh,r);}
function gexPhoneObj_(x){return {ticker:x[0],qty:x[1],costBasis:x[2],costPerShare:x[3],liveSpot:x[4],callWall:x[5],putWall:x[6],gammaFlip:x[7],takeProfit:x[8],tpPct:gexPctFromCost_(x[8],x[3]),stopLimit:x[10],slPct:gexPctFromCost_(x[10],x[3])};}
