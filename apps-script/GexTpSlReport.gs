...
function buildGexTakeProfitStopLimit(){
  ...
  return 'GEX TP & SL complete. Main rows shown: '+rows.length+'. Phone-Gmail tab updated with printable table and EST. Loss.'+(errors.length?' Hidden/issues: '+errors.length+'.':'');
}
...
function gexEstLoss_(sl,cost,qty){var s=gexNum_(sl),c=gexNum_(cost),q=gexNum_(qty);return (!s||!c||!q)?'n/a':((s-c)*q).toLocaleString('en-US',{style:'currency',currency:'USD',minimumFractionDigits:2,maximumFractionDigits:2});}
