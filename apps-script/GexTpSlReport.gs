var GEX_TPSL={holdingsSheet:'Holdings',reportSheet:'GEX Take Profit & Stop Limit',phoneSheet:'Phone-Gmail',font:'Times New Roman',maxTickers:40,maxDte:45,minOi:1,cboeBase:'https://cdn.cboe.com/api/global/delayed_quotes/options/'};
...
function buildPhoneGmailGexReport(){
  ...
  var ss=SpreadsheetApp.getActiveSpreadsheet(),source=ss.getSheetByName(GEX_TPSL.reportSheet);
  if(!source) throw new Error('Missing GEX Take Profit & Stop Limit tab.');
  var rows=gexPhoneReadRows_(source);
  if(!rows.length) throw new Error('No usable GEX TP/SL rows found for Phone-Gmail.');
  var sh=ss.getSheetByName(GEX_TPSL.phoneSheet)||ss.insertSheet(GEX_TPSL.phoneSheet);
  gexPhonePrepare_(sh);
  var r=1;
  r=gexPhoneTitle_(sh,r,'GEX TP / SL','Phone-Gmail view • '+gexNow_());
  r=gexPhoneSummary_(sh,r,rows.length);
  rows.forEach(function(x,i){r=gexPhoneCard_(sh,r,x,i+1);});
  gexPhoneFinalize_(sh,r);
  return 'Phone-Gmail report complete. Cards: '+rows.length+'. Check the Phone-Gmail tab.';
}

function buildGexTakeProfitStopLimitAndPhoneGmail(){
  buildGexTakeProfitStopLimit();
  return buildPhoneGmailGexReport();
}
...
function gexPhoneReadRows_(source){SpreadsheetApp.flush();Utilities.sleep(750);var values=source.getDataRange().getDisplayValues(),hr=-1;for(var i=0;i<values.length;i++){if(values[i].indexOf('Ticker')>=0&&values[i].indexOf('Take Profit')>=0&&values[i].indexOf('Stop Limit')>=0){hr=i;break;}}if(hr<0)return[];var headers=values[hr],idx={};headers.forEach(function(h,i){idx[String(h||'').trim()]=i;});var out=[];for(var r=hr+1;r<values.length;r++){var row=values[r],ticker=String(row[idx['Ticker']]||'').trim();if(!ticker||ticker==='Hidden / Data Issues'||ticker==='Data Issues')break;var tp=gexPhoneClean_(row[idx['Take Profit']]),sl=gexPhoneClean_(row[idx['Stop Limit']]);if(gexPhoneIsNa_(tp)&&gexPhoneIsNa_(sl))continue;out.push({ticker:ticker,qty:gexPhoneClean_(row[idx['Qty']]),costBasis:gexPhoneClean_(row[idx['Cost Basis']]),costPerShare:gexPhoneClean_(row[idx['Cost / Share']]),liveSpot:gexPhoneClean_(row[idx['Live Spot']]),callWall:gexPhoneClean_(row[idx['Call Wall']]),putWall:gexPhoneClean_(row[idx['Put Wall']]),gammaFlip:gexPhoneClean_(row[idx['Gamma Flip']]),takeProfit:tp,tpPct:gexPhoneClean_(row[idx['TP %']]),stopLimit:sl,slPct:gexPhoneClean_(row[idx['SL %']])});}out.sort(function(a,b){return gexPhonePctNum_(b.tpPct)-gexPhonePctNum_(a.tpPct);});return out;}
function gexPhonePrepare_(sh){try{sh.showColumns(1,sh.getMaxColumns());}catch(e){}sh.getRange(1,1,sh.getMaxRows(),sh.getMaxColumns()).breakApart();sh.clear();sh.setHiddenGridlines(true);if(sh.getMaxColumns()<6)sh.insertColumnsAfter(sh.getMaxColumns(),6-sh.getMaxColumns());sh.getRange(1,1,sh.getMaxRows(),6).setFontFamily(GEX_TPSL.font).setFontSize(11).setWrap(true).setVerticalAlignment('middle');}
function gexPhoneTitle_(sh,row,title,sub){sh.getRange(row,1,1,6).merge().setValue(title).setFontSize(22).setFontWeight('bold').setFontColor('#ffffff').setBackground('#111827').setHorizontalAlignment('center');sh.setRowHeight(row,38);row++;sh.getRange(row,1,1,6).merge().setValue(sub).setFontSize(10).setFontColor('#e5e7eb').setBackground('#374151').setHorizontalAlignment('center');sh.setRowHeight(row,24);return row+2;}
function gexPhoneSummary_(sh,row,count){sh.getRange(row,1,1,6).merge().setValue('iPhone/Gmail view: TP and SL side-by-side; TP % and SL % side-by-side. Percentages are from cost/share.').setFontSize(10).setFontColor('#111827').setBackground('#fef3c7').setHorizontalAlignment('center');sh.setRowHeight(row,42);row++;sh.getRange(row,1,1,6).merge().setValue('Cards shown: '+count+' • Sorted highest TP % to lowest').setFontSize(10).setFontColor('#111827').setBackground('#dbeafe').setHorizontalAlignment('center');sh.setRowHeight(row,26);return row+2;}
function gexPhoneCard_(sh,row,x,rank){var green='#dcfce7',red='#fee2e2',blue='#dbeafe',gray='#f3f4f6',dark='#1f2937';sh.getRange(row,1,1,6).merge().setValue(rank+'. '+x.ticker).setFontSize(18).setFontWeight('bold').setFontColor('#ffffff').setBackground(dark).setHorizontalAlignment('center');sh.setRowHeight(row,30);row++;sh.getRange(row,1,1,2).merge().setValue('Qty\n'+x.qty).setBackground(gray).setFontWeight('bold').setHorizontalAlignment('center');sh.getRange(row,3,1,2).merge().setValue('Cost/Share\n'+x.costPerShare).setBackground(gray).setFontWeight('bold').setHorizontalAlignment('center');sh.getRange(row,5,1,2).merge().setValue('Live Spot\n'+x.liveSpot).setBackground(gray).setFontWeight('bold').setHorizontalAlignment('center');sh.setRowHeight(row,48);row++;sh.getRange(row,1,1,3).merge().setValue('TAKE PROFIT').setBackground(green).setFontWeight('bold').setHorizontalAlignment('center');sh.getRange(row,4,1,3).merge().setValue('STOP LIMIT').setBackground(red).setFontWeight('bold').setHorizontalAlignment('center');sh.setRowHeight(row,24);row++;sh.getRange(row,1,1,3).merge().setValue(x.takeProfit).setBackground('#bbf7d0').setFontSize(17).setFontWeight('bold').setHorizontalAlignment('center');sh.getRange(row,4,1,3).merge().setValue(x.stopLimit).setBackground('#fecaca').setFontSize(17).setFontWeight('bold').setHorizontalAlignment('center');sh.setRowHeight(row,34);row++;sh.getRange(row,1,1,3).merge().setValue(x.tpPct).setBackground(green).setFontSize(15).setFontWeight('bold').setHorizontalAlignment('center');sh.getRange(row,4,1,3).merge().setValue(x.slPct).setBackground(red).setFontSize(15).setFontWeight('bold').setHorizontalAlignment('center');sh.setRowHeight(row,30);row++;sh.getRange(row,1,1,2).merge().setValue('Call Wall\n'+x.callWall).setBackground(blue).setHorizontalAlignment('center');sh.getRange(row,3,1,2).merge().setValue('Put Wall\n'+x.putWall).setBackground(blue).setHorizontalAlignment('center');sh.getRange(row,5,1,2).merge().setValue('G-Flip\n'+x.gammaFlip).setBackground(blue).setHorizontalAlignment('center');sh.setRowHeight(row,46);sh.getRange(row-5,1,6,6).setBorder(true,true,true,true,true,true,'#9ca3af',SpreadsheetApp.BorderStyle.SOLID);return row+2;}
function gexPhoneFinalize_(sh,lastRow){for(var c=1;c<=6;c++)sh.setColumnWidth(c,70);if(sh.getMaxColumns()>6)sh.hideColumns(7,sh.getMaxColumns()-6);sh.setFrozenRows(0);sh.getRange(1,1,Math.max(lastRow,1),6).setVerticalAlignment('middle');}
function gexPhoneClean_(v){var s=String(v===null||v===undefined?'':v).trim();return s||'n/a';}
function gexPhoneIsNa_(v){var s=String(v||'').trim().toLowerCase();return !s||s==='n/a'||s==='na'||s==='#n/a';}
function gexPhonePctNum_(v){var n=Number(String(v||'').replace('%','').replace(',','').trim());return isNaN(n)?-999999:n;}
