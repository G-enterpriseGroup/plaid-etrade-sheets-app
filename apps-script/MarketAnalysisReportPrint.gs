/**
 * Market Analysis Report print helper.
...
function mrPrintCleanPrintedReport_() {
  ...
  mrPrintRemoveSourceList_(sh, lastRow);
  lastRow = Math.max(1, sh.getLastRow());
  mrPrintStandardizeWords_(sh, lastRow, lastCol);
  mrPrintAddTrimPnlRows_(sh);
  mrPrintFinalPortraitPolish_(sh);
}

function mrPrintRemoveSourceList_(sh, lastRow) {
...
function mrPrintStandardizeWords_(sh, lastRow, lastCol) {
...
function mrPrintAddTrimPnlRows_(sh) {
  var trimLabel = 'Tr' + 'im-QTY';
  var row = 1;
  while (row <= sh.getLastRow()) {
    ...
    if (title.indexOf(trimLabel) >= 0) {
      ...
      var dataRow = row + 2;
      var noteRow = row + 5;
      var totalQty = mrPrintNum_(sh.getRange(dataRow, 1).getDisplayValue());
      var totalPnl = mrPrintNum_(sh.getRange(dataRow, 3).getDisplayValue());
      ...
      if (nextText.indexOf('Estimated P&L on this trim') < 0) {
        ...
        sh.getRange(noteRow, 1, 1, 6).merge()
          .setValue('Estimated P&L on this trim: ' + mrPrintMoney_(estTrimPnl) + '  |  Math: action quantity × average unrealized P&L per share. This is an estimate, not a realized-tax calculation.')
...
function mrPrintFinalPortraitPolish_(sh) {
